    ...
    <header className="sticky top-0 z-40 bg-[#d0e2f1]/95 backdrop-blur-md border-b border-sky-200/60 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center space-x-1 text-xs font-bold text-[#486379]">
          ...
          {/* VISIBLE SOLO SI EL ROL AUTENTICADO ES ADMIN */}
          {userRole === 'ADMIN' && (
        ...
        <div className="hidden md:flex items-center space-x-2">
          {userRole === 'GUEST' || !userRole ? (
            ...
            <button onClick={() => navigateTo(userRole === 'ADMIN' ? 'admin' : 'home')} className="px-4 py-2 bg-white text-[#486379] text-xs font-bold rounded-full border border-sky-100 cursor-pointer">
              {userRole === 'ADMIN' ? 'Panel Admin' : `Mi Cuenta (${userName})`}
            </button>
          )}
        </div>
