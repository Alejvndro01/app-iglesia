...
export function RegisterView({ navigateTo, showToast }: Omit<AuthProps, 'setUserRole'>) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Error de conexión al registrar usuario');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-8 border border-sky-100 dark:border-slate-800 transition-colors">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-[#486379] hover:bg-[#385063] text-white font-bold text-xs rounded-full shadow-md cursor-pointer disabled:opacity-50"
          >
